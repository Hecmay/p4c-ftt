...
class StateTranslationVisitor : public CodeGenInspector {
 ...
 public:
    explicit StateTranslationVisitor(const P4HLSParserState* state) :
...
StateTranslationVisitor::compileExtractField(
    const IR::Expression* expr, cstring field, unsigned alignment, P4HLSType* type) {
    ...
    if (widthToExtract <= 64) {
        ...
        if (shift == 0)
            ...
            helper = "load_half";
        auto bt = P4HLSTypeFactory::instance->create(IR::Type_Bits::get(8));
...
void P4HLSParserState::emit(CodeBuilder* builder) {
    StateTranslationVisitor visitor(this);
    visitor.setBuilder(builder);
    state->apply(visitor);
}

P4HLSParser::P4HLSParser(const P4HLSProgram* program, const IR::ParserBlock* block,
                       const P4::TypeMap* typeMap) :
        program(program), typeMap(typeMap), parserBlock(block),
        packet(nullptr), headers(nullptr), headerType(nullptr) {}

void P4HLSParser::emitDeclaration(CodeBuilder* builder, const IR::Declaration* decl) {
...
void P4HLSParser::emit(CodeBuilder* builder) {
...
bool P4HLSParser::build() {
    ...
    for (auto state : parserBlock->container->states) {
        auto ps = new P4HLSParserState(state, this);
        states.push_back(ps);
    }
    ...
    if (ht == nullptr)
        return false;
    headerType = P4HLSTypeFactory::instance->create(ht);
    return true;
}
