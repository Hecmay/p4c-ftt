...
bool P4HLSProgram::build() {
    ...
    parser = new P4HLSParser(this, pb, typeMap);
    ...
    control = new P4HLSControl(this, cb, parser->headers);
...
void P4HLSProgram::emitC(CodeBuilder* builder, cstring header) {
...
void P4HLSProgram::emitGeneratedComment(CodeBuilder* builder) {
...
void P4HLSProgram::emitH(CodeBuilder* builder, cstring) {
    emitGeneratedComment(builder);
...
void P4HLSProgram::emitTypes(CodeBuilder* builder) {
    for (auto d : program->objects) {
        if (d->is<IR::Type>() && !d->is<IR::IContainer>() &&
            ...
            !d->is<IR::Type_Error>()) {
            auto type = P4HLSTypeFactory::instance->create(d->to<IR::Type>());
            if (type == nullptr)
                continue;
            type->emit(builder);
            builder->newline();
...
void P4HLSProgram::emitPreamble(CodeBuilder* builder) {
    builder->emitIndent();
    ...
    builder->endOfStatement(true);
    builder->newline();
    builder->appendLine("#define P4HLS_MASK(t, w) ((((t)(1)) << (w)) - (t)1)");
    builder->appendLine("#define BYTES(w) ((w) / 8)");
    builder->appendLine(
        "#define write_partial(a, s, v) do "
        "{ u8 mask = P4HLS_MASK(u8, s); "
...
void P4HLSProgram::emitLocalVariables(CodeBuilder* builder) {
    builder->emitIndent();
...
void P4HLSProgram::emitHeaderInstances(CodeBuilder* builder) {
    builder->emitIndent();
    parser->headerType->declare(builder, parser->headers->name.name, false);
}

void P4HLSProgram::emitPipeline(CodeBuilder* builder) {
    builder->emitIndent();
