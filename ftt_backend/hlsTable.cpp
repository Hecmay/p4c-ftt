...
class ActionTranslationVisitor : public CodeGenInspector {
 ...
 public:
    ActionTranslationVisitor(cstring valueName, const P4HLSProgram* program):
    ...
    bool preorder(const IR::PathExpression* expression) {
        ...
        if (decl->is<IR::Parameter>()) {
            ...
            if (isParam) {
                ...
                builder->append("->u.");
                cstring name = P4HLSObject::externalName(action);
                builder->append(name);
                builder->append(".");
...
P4HLSTable::P4HLSTable(const P4HLSProgram* program, const IR::TableBlock* table,
                     CodeGenInspector* codeGen) :
        P4HLSTableBase(program, P4HLSObject::externalName(table->container), codeGen), table(table) {
...
void P4HLSTable::emitKeyType(CodeBuilder* builder) {
    builder->emitIndent();
    ...
    if (keyGenerator != nullptr) {
        ...
        for (auto c : keyGenerator->keyElements) {
            ...
            auto ebpfType = P4HLSTypeFactory::instance->create(type);
...
void P4HLSTable::emitActionArguments(CodeBuilder* builder,
    ...
    for (auto p : *action->parameters->getEnumerator()) {
        builder->emitIndent();
        auto type = P4HLSTypeFactory::instance->create(p->type);
...
void P4HLSTable::emitValueType(CodeBuilder* builder) {
...
void P4HLSTable::emitTypes(CodeBuilder* builder) {
    emitKeyType(builder);
    emitValueType(builder);
}

void P4HLSTable::emitInstance(CodeBuilder* builder) {
    if (keyGenerator != nullptr) {
        ...
        cstring name = P4HLSObject::externalName(table->container);
...
void P4HLSTable::emitKey(CodeBuilder* builder, cstring keyName) {
    if (keyGenerator == nullptr)
        return;
    for (auto c : keyGenerator->keyElements) {
        auto ebpfType = ::get(keyTypes, c);
        ...
        bool memcpy = false;
        P4HLSScalarType* scalar = nullptr;
        unsigned width = 0;
        if (ebpfType->is<P4HLSScalarType>()) {
            scalar = ebpfType->to<P4HLSScalarType>();
            width = scalar->implementationWidthInBits();
            memcpy = !P4HLSScalarType::generatesScalar(width);
        }
...
void P4HLSTable::emitAction(CodeBuilder* builder, cstring valueName) {
    builder->emitIndent();
...
void P4HLSTable::emitInitializer(CodeBuilder* builder) {
...
P4HLSCounterTable::P4HLSCounterTable(const P4HLSProgram* program, const IR::ExternBlock* block,
                                   cstring name, CodeGenInspector* codeGen) :
        P4HLSTableBase(program, name, codeGen) {
...
void P4HLSCounterTable::emitInstance(CodeBuilder* builder) {
...
void P4HLSCounterTable::emitCounterIncrement(CodeBuilder* builder,
...
void
P4HLSCounterTable::emitMethodInvocation(CodeBuilder* builder, const P4::ExternMethod* method) {
...
void P4HLSCounterTable::emitTypes(CodeBuilder* builder) {
    builder->emitIndent();
    builder->appendFormat("typedef %s %s",
                          P4HLSModel::instance.counterIndexType.c_str(), keyTypeName.c_str());
    builder->endOfStatement(true);
    builder->emitIndent();
    builder->appendFormat("typedef %s %s",
                          P4HLSModel::instance.counterValueType.c_str(), valueTypeName.c_str());
    builder->endOfStatement(true);
}
