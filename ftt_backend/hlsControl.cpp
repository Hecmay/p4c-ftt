...
ControlBodyTranslator::ControlBodyTranslator(const P4HLSControl* control) :
...
void ControlBodyTranslator::compileEmit(const IR::Vector<IR::Argument>* args) {
    ...
    for (auto f : ht->fields) {
        ...
        auto etype = P4HLSTypeFactory::instance->create(ftype);
...
void ControlBodyTranslator::processMethod(const P4::ExternMethod* method) {
    ...
    if (declType->name.name == P4HLSModel::instance.counterArray.name) {
        builder->blockStart();
        cstring name = P4HLSObject::externalName(decl);
...
bool ControlBodyTranslator::preorder(const IR::SwitchStatement* statement) {
    ...
    for (auto c : statement->cases) {
        ...
        if (c->label->is<IR::DefaultExpression>()) {
            ...
            cstring name = P4HLSObject::externalName(act);
            builder->append(name);
        }
        builder->append(":");
        builder->newline();
    ...
    for (auto c : controlBlock->constantValue) {
        ...
        if (b->is<IR::TableBlock>()) {
            ...
            if (node->is<IR::Declaration_Instance>()) {
                ...
                cstring name = P4HLSObject::externalName(di);
                auto ctr = new P4HLSCounterTable(program, ctrblk, name, codeGen);
...
void P4HLSControl::emitDeclaration(CodeBuilder* builder, const IR::Declaration* decl) {
    if (decl->is<IR::Declaration_Variable>()) {
        auto vd = decl->to<IR::Declaration_Variable>();
        auto etype = P4HLSTypeFactory::instance->create(vd->type);
        builder->emitIndent();
...
void P4HLSControl::emit(CodeBuilder* builder) {
    auto hitType = P4HLSTypeFactory::instance->create(IR::Type_Boolean::get());
    builder->emitIndent();
...
void P4HLSControl::emitTableTypes(CodeBuilder* builder) {
    for (auto it : tables)
...
void P4HLSControl::emitTableInstances(CodeBuilder* builder) {
    for (auto it : tables)
...
void P4HLSControl::emitTableInitializers(CodeBuilder* builder) {
    for (auto it : tables)
